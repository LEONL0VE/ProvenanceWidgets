...
/**
 * Resolve a widget's value at one point in the SuperProvenance timeline.
 *
 * This mirrors PW 1.0's aggregate/temporal click behavior, but uses the
 * registration type instead of guessing from the widget id.
 */
...
export const buildSuperAggregateBoxes = ({
    ...
    // Match the original Super_Version2 design:
    // - every zero-interaction widget keeps a fixed empty outline slot;
    // - interacted widgets divide only the remaining track.
    return widgetIds
        .map(widgetId => {
