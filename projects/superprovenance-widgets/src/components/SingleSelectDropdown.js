...
import { transform, getScentColor, getContrastColor } from './utils.js';
...
const SingleSelectItem = ({ option, guidance, showTimeline }) => {
    ...
    }, [showTimeline, guidance, option.value]);

    return (
        <div ref={containerRef} style={{ position: 'relative', height: '32px', display: 'flex', alignItems: 'center', padding: '0 8px', width: '100%', flex: 1 }}>
            ...
            {showTimeline && timelineData && (
                <div style={{ position: 'absolute', inset: 0, zIndex: 0, display: 'flex', alignItems: 'center' }}>
                    <TimelineVis records={timelineData.records} maxIndex={timelineData.maxIndex} />
                </div>
            )}
            ...
            <span style={{
                ...
                zIndex: 1,
                pointerEvents: 'none',
                color: showTimeline ? '#000' : getContrastColor(getScentColor(option.value, guidance, interpolateOranges))
...
const SingleSelectDropdown = (props) => {
    ...
    const itemTemplate = (option) => {
        return <SingleSelectItem option={option} guidance={provRef.current} showTimeline={showTimeline} />;
    };
