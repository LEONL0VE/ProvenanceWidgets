...
const Radiobutton = ({ label, value, stateItem, setStateItem }) => {
    ...
    }, [showTimeline, radioGroup?.guidance, value]);
    ...
    return (
        <div style={{ display: "flex", alignItems: "center", gap: "5px", marginTop: "1rem", width: "100%" }}>
            ...
            <div ref={setCombinedRef} style={{ position: "relative", flex: 1, display: 'flex', alignItems: 'center' }}>
                ...
                {showTimeline && timelineData && (
                    <div style={{ position: 'absolute', inset: 0, zIndex: 0 }}>
                        <TimelineVis records={timelineData.records} maxIndex={timelineData.maxIndex} leftInsetPx={axisOffsetPx} />
                    </div>
                )}
                ...
                <label htmlFor={value} className="ml-2" style={{
                    ...
                    display: "block",
                    padding: "2px 8px",
                    // Use getContrastColor logic if timeline is NOT shown.
                    // If timeline IS shown, the background is the orange bars (#FFC107).
                    // Black on #FFC107 is readable.
                    // But if the user means the default scent bar (orange gradient), we should use getContrastColor.
                    // The previous change hardcoded it to black.
                    // Let's revert to using getContrastColor for the default view, 
                    // and keep black for the timeline view (unless timeline bars are dark? No, they are yellow).
                    color: showTimeline ? '#000000' : getContrastColor(getScentColor(value, radioGroup?.guidance, interpolateOranges)),
