    ...
    const chartData = useMemo(() => {
        ...
        if (isRangedProvenance) {
            ...
            const transformedData = sorted.map(([key, record]) => {
                ...
                return [label, [{ select: { index: min }, unselect: { index: max } }]];
            });
        ...
        if (isTextProvenance) {
            ...
            if (sorted.length > 0) {
                ...
                grouped.get(currentVal).push({ select: { index: startIndex, time: sorted[0][1].time } });
                ...
                for (let i = 1; i < sorted.length; i++) {
                    ...
                    if (nextVal !== currentVal) {
                        ...
                        if (currentRecords && currentRecords.length > 0) {
                            currentRecords[currentRecords.length - 1].unselect = { index: nextIndex, time: nextRec.time };
                        }
                        
                        // Open next
                        if (!grouped.has(nextVal)) grouped.set(nextVal, []);
                        grouped.get(nextVal).push({ select: { index: nextIndex, time: nextRec.time } });
            ...
            const effectiveMax = Math.max(currentDomainMax, dataMaxIndex) + 1;
                                              ...
                                              // Render Points ONLY
                                               return (
                                                  <div key={i} style={{ position: 'absolute', top: 0, left: 0, width: '100%', height: '100%' }}>
                                                        {/* Min Point */}
                                                        <div 
                                                            title={chartData.isSingleSlider ? `${start}` : `Min: ${start}`}
                                                            style={{
                                                                position: 'absolute',
                                                                left: `calc(${left}% - 4px)`, 
                                                                top: '0px',
                              ...
                              <div key={label} style={{ display: 'flex', alignItems: 'center', height: '24px', position: 'relative' }}>
                                  ...
                                  <div style={{ position: 'absolute', top: 0, left: 0, width: '100%', height: '100%', background: rowBg, borderRadius: '3px', zIndex: 0 }}>
                                      {records.map((record, i) => {
                                           ...
                                           return (
                                               <div key={i} 
                                                  title={`Start: ${Math.max(0, start)}, End: ${Math.max(0, end)}`}
                                                  style={{
                                                   position: 'absolute',
                                                   left: `${left}%`,
                                                   width: `${width}%`,
